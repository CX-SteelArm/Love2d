...
-- values 1: min 2: max 3: current value 4: step 5: axis(horizontal or vertical)
...
Gspot.collapsegroup = {
	load = function(this, Gspot, label, pos, parent)
		...
		element.toggle = function(this)
			...
			for i, child in ipairs(this.children) do
				if child ~= this.control and not child.nohide then
...
---[[
Gspot.dytext = {
	load = function(this, Gspot, variable, pos, parent, autosize)
    local label = _G[variable]
		local element = Gspot:element('dytext', label, pos, parent)
    element.va = variable
		if autosize then
			element.pos.w = element.style.font:getWidth(label) + (element.style.unit / 2)
			element.autosize = autosize
...
setmetatable(Gspot.dytext, {__index = Gspot.util, __call = Gspot.dytext.load})
